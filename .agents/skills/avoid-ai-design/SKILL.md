...
A page can pass the catalog and still fail all three. The catalog catches cliches; these tests catch mediocrity.
## Severity tiers
...
- **Do not trade one cliche for another.** The "Space Grotesk trap": models reach for the same "tasteful" non-default every time (Space Grotesk, a slate palette, one stock gradient). A second-order default is still a default. Vary your choices across runs and justify them by context.
